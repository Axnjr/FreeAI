...
export default function ChatDemo({ test }: { test?: string | boolean }) {
    ...
    async function query(data: { inputs: string }) {
        ...
        if (data["inputs"]) {
            ...
            try {
                const response = await fetch(`/api/chatConvo?query=${data["inputs"]}&test_API_KEY=${test}`);
                const result = await response.json();

                setChatMessage(result["generated_text"])
            } 
            catch (error) {
                setChatMessage("Error occured please try after refreshing the page")
            }
